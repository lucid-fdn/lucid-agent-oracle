...
1. **Poll** — Query gateway Postgres tables using durable compound watermarks `(last_seen_ts, last_seen_id)` per table. Read the `watermark_column` from `oracle_worker_checkpoints` for each table (defaults to `created_at`; `gateway_payment_sessions` uses `updated_at`). Fetch rows with lexicographic `(watermark_column, id) > (checkpoint_ts, checkpoint_id)`. When a payment session is re-fetched due to an `updated_at` change, the worker produces a **new event with a new `event_id`** (deterministic from the current row state) and sets `corrects_event_id` to the previous event's deterministic `event_id` (computed from the same session ID with the prior state). The MV's `WHERE corrects_event_id IS NULL` filter excludes the correction event from rollups, preventing double-counting. Only the first observation of each payment session contributes to rollup aggregates. The full state-change history is preserved in `raw_economic_events` for future reconciliation when settlement data is available.
